...
def gen_tile(
    ...
    if not b3dm:
        ...
        run(
            f"obj23dtiles -i {op.abspath(tmp_cropped_obj)} -b --b3dm -o {b3dm_tmp_path}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", help="path to .obj file (that links to .mtl & .png)", type=str
    )
    parser.add_argument("--output", help="path to .b3dm or .glb file", type=str)

    parser.add_argument(
        "--crop",
        help="true = crop using min/max x/y/z, false = no cropping",
        default=False,
    )
    parser.add_argument(
        "--b3dm", help="true = generate b3dm, false = generate glb", default=False
    )
    ...
    parser.add_argument(
        "--quality",
        help="number between 1 (worst) and 99 (best)",
        type=str,
        default="50",
    )
    parser.add_argument(
        "--scale",
        help="number between 1 (smallest) and 99 (largest)",
        type=str,
        default="10",
    )
